...
This set of ODEs does have an analytical solution [(Harko et al 2014)](https://arxiv.org/abs/1403.2160) - but for the purpose of interpretability and to mirror how we are reporting cases (that is, on a daily basis), I instead decided to run a simulation. I have approximated these equations with $\Delta s/\Delta t$,  $\Delta i/\Delta t$, and $\Delta r/\Delta t$, respectively - where $\Delta t = 1$ (day). On each day $t$, $s_t$, $i_t$, and $r_t$ are updated accordingly:
$$s_{t+1} = s_t - \beta s_t i_t$$
$$i_{t+1} = (\beta s_t - \gamma + 1)i_t$$
$$r_{t+1} = (\gamma + 1) i_t$$